    ...
    class Bouncer extends GraphicalObject
    {
       ...
       enlarge()
       {
          this.bouncer_radius  = this.bouncer_radius + 20 ;
       }
    ...
    // move the ball when space is pressed
    
    var game_is_being_played = false
    function on_space_press(event){
         if (event.which == 32){
    ...
    function run_this_application()
    {
      ...
      if(game_is_being_played == true){       // game is being played true
       ball_on_screen.move() ;
      }
       console.log(game_is_being_played);
       draw_on_canvas() ;
...
for ( var ball_counter = 0 ;
   ...
   ball_counter ++ )
   {
   var new_ball = new ExplodingBouncer( 400, 300, ball_colors[ball_counter],
   bouncing_area ) ;
   game_balls.push( new_ball ) ;
   }

//to give different balls different color
